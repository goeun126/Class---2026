...
# 따라서 가볍게 훑어보고 필요할 때 찾아서 사용하면 됨.

# math 모듈: 수학 함수와 관랸된 모듈
# fads(): 절댓값을 반환
...
print(f'math.fabs(-3.1415): {math.fabs(-3.1415)}') # 절대 값
...
print(f'math.factorial(7): {math.factorial(7)}') # 버림
print(f'math.sqrt(9): {math.sqrt(9)}') # 제곱근

# 파이썬 내장 함수 중에서 수학과 관련된 함수가 있음
# sum(): 전체 합을 반환
# max(): 최대값을 반환
# min(): 최소값을 반환
# abs(): 절댓닶을 반환
...
# random(랜덤) 모듈: 난수를 발생시키는 모듈임.
# random(): 0이상 1미만의 난수 발생
# randint(n1, n2): n1 이상 n2이하의 난수를 발생
# randrange(n1, n2): n1 이상 n2미만의 난수를 발생
# sample(range(n1, n2), n3): n1이상 n2 미만의 난수 n3개 발생(중복 없이)
